...
export default function DetailPage() {
  ...
  if (!account) return <div className="min-h-screen bg-[#0F172A] flex items-center justify-center text-red-500">404</div>;
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white pb-28 font-sans">
      ...
      {isImageOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md animate-fade-in cursor-zoom-out" onClick={() => setIsImageOpen(false)}>
            <button className="absolute top-4 right-4 p-3 bg-white/10 rounded-full text-white hover:bg-white/20 transition-all"><X size={24} /></button>
            <img src={account.image_url} alt={account.title} className="max-w-full max-h-full rounded-lg shadow-2xl object-contain scale-100" onClick={(e) => e.stopPropagation()}/>
        </div>
      )}

      <div className="fixed top-0 left-0 right-0 p-4 z-30 flex justify-between items-center pointer-events-none">
        <div className="pointer-events-auto flex gap-2">
            <button onClick={() => navigate(-1)} className="p-3 bg-[#0F172A]/60 backdrop-blur-md rounded-full hover:bg-white/10 border border-white/10 shadow-lg"><ArrowLeft size={20} /></button>
        </div>
      ...
      <div className="relative w-full h-[50vh] group cursor-pointer" onClick={() => setIsImageOpen(true)}>
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/20 backdrop-blur-[2px]"><div className="bg-black/50 p-3 rounded-full text-white border border-white/20 backdrop-blur-md"><ZoomIn size={32} /></div></div>
      ...
      <div className="px-6 -mt-16 relative z-10">
        <div className="flex flex-wrap gap-2 mb-4">
            <span className="bg-violet-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold flex items-center gap-1.5 shadow-lg shadow-violet-600/40 border border-violet-400/20"><Gamepad2 size={14} /> {account.games?.name}</span>
            <span className="bg-cyan-500/10 text-cyan-400 px-3 py-1.5 rounded-xl text-[10px] font-bold flex items-center gap-1.5 border border-cyan-500/20 backdrop-blur-md"><ShieldCheck size={14} /> Verified Seller</span>
        </div>
        ...
        <div className="bg-[#1E293B]/80 backdrop-blur-xl p-5 rounded-3xl border border-white/10 mb-8 mt-6 shadow-2xl flex justify-between items-center relative overflow-hidden group">
           ...
           <div className="relative z-10">
              ...
              <p className="text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-300">Rp {parseInt(account.price).toLocaleString('id-ID')}</p>
           </div>
           <div className="relative z-10 text-right pl-6 border-l border-white/10">
              ...
              <div className="flex items-center gap-3 justify-end">
                  ...
                  <div className="w-10 h-10 bg-gradient-to-tr from-violet-600 to-fuchsia-600 rounded-full flex items-center justify-center text-sm font-bold shadow-lg ring-2 ring-[#0F172A]">{account.sellers?.name?.charAt(0)}</div>
              </div>
           </div>
        </div>

        <div className="mb-10">
          <h3 className="font-bold text-lg mb-4 text-white flex items-center gap-3"><div className="w-1.5 h-6 bg-cyan-500 rounded-full shadow-[0_0_10px_cyan]"></div> Spesifikasi Akun</h3>
          <div className="text-slate-300 text-sm leading-relaxed whitespace-pre-line bg-[#1E293B]/40 p-6 rounded-3xl border border-white/5 shadow-inner">{account.description}</div>
        </div>

        {isOwner && !account.is_sold && <button onClick={() => setShowSoldModal(true)} className="w-full py-4 border-2 border-dashed border-slate-600 text-slate-400 font-bold rounded-2xl hover:border-red-500 hover:text-red-500 hover:bg-red-500/5 transition-all text-sm uppercase tracking-wider mb-4">Tandai Sebagai Terjual</button>}
      </div>

      <div className="fixed bottom-0 left-0 right-0 bg-[#0F172A]/80 backdrop-blur-xl p-5 border-t border-white/5 z-40 pb-safe">
        <div className="max-w-2xl mx-auto">
            <button onClick={handleBuy} disabled={account.is_sold} className={`w-full font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl text-lg transition-all active:scale-95 ${account.is_sold ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-white/5' : 'bg-gradient-to-r from-emerald-500 to-green-600 text-white hover:shadow-emerald-500/20'}`}>
